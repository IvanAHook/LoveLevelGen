...
function room.generate_room( room_type, room_x, room_y )
	...
	local objects = {}
	local template = templates.get_template( "1" )

	for k,v in pairs( template ) do
		if v.block == 1 then
            table.insert( objects,
                new_block( tile_base*( v.x + 0.5 )+room_offset_x, tile_base*( v.y + 0.5 )+room_offset_y, "static", { 0, 0, 0 }, nil ) )
		elseif v.block == 2 then
            -- no block 2 atm
		elseif  v.block == "L" then
            table.insert( objects,
                new_block( tile_base*( v.x + 0.5 )+room_offset_x, tile_base*( v.y + 0.5 )+room_offset_y, "static", { 255, 50, 50 }, nil ) )
		elseif v.block == "R" then
            local rand_int = nil
            rand_int = love.math.random( 0, 3 )
 			if rand_int == 1 then
                table.insert( objects,
                    new_block( tile_base*( v.x + 0.5 )+room_offset_x, tile_base*( v.y + 0.5 )+room_offset_y, "dynamic", { 255, 255, 255 }, box_image, 0, 1 ) )
            else
                table.insert( objects,
                    new_block( tile_base*( v.x + 0.5 )+room_offset_x, tile_base*( v.y + 0.5 )+room_offset_y, "static", { 255, 50, 255 }, nil ) )
            end
        end
